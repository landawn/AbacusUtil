...
public class DBUtil {
    ...
    public PreparedQuery prepareQuery(final String sql) throws SQLException {
        final Connection conn = getConnectionInSpring(ds);
        ...
        try {
            ...
            if (result == null && conn != null) {
                releaseConnectionInSpring(conn, ds);
            }
        }
    ...
    private static Connection getConnectionInSpring(final DataSource ds) {
        return DataSourceUtils.getConnection(ds);
    }

    private static void releaseConnectionInSpring(final Connection conn, final DataSource ds) {
    ...
    public static interface Dao<T> extends JdbcUtil.Dao<T> {
        ...
        default PreparedQuery prepareQuery(final Try.Function<Connection, PreparedStatement, SQLException> stmtCreator) throws SQLException {
            ...
            try {
                result = JdbcUtil.prepareQuery(conn, stmtCreator).onClose(() -> DataSourceUtils.releaseConnection(conn, ds));
            } finally {
                if (result == null && conn != null) {
                    releaseConnectionInSpring(conn, ds);
                }
        ...
        default PreparedCallableQuery prepareCallableQuery(final Try.Function<Connection, CallableStatement, SQLException> stmtCreator) throws SQLException {
            ...
            try {
                result = JdbcUtil.prepareCallableQuery(conn, stmtCreator).onClose(() -> DataSourceUtils.releaseConnection(conn, ds));
            } finally {
                if (result == null && conn != null) {
                    releaseConnectionInSpring(conn, ds);
                }
